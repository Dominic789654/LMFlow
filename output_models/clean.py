...
def remove_redundant_checkpoints(args):
    ...
    for checkpoint in checkpoint_dir.iterdir():
        ...
        try:
          # 定义正则表达式模式，以匹配特定的格式并捕获尾随的数字
            pattern = r'^ft_Llama2-70B_GSM8K_lr5e-5_lisa_random4_openEmdAndHead_k100_3epoch_(\d+)$'

            # 使用正则表达式匹配模式并捕获检查点ID

            match = re.search(pattern, (str(checkpoint).split("/")[-1]))
            if match:
                checkpoint_id = int(match.group(1))  # 提取ID
                checkpoint_id_list.append(checkpoint_id)

            else:
                checkpoint_id = None  # 如果没有匹配，设置ID为None

            checkpoint_id  # 这将是提取的数字ID
        except:
            continue
    ...
    if args.debug_mode:
        for checkpoint_id in sorted(remove_id_list):
            checkpoint_path = checkpoint_dir / f'ft_phi2_adamw_gpt4_v2_1e-5_activate_random*2_no_1_k3_warm0.06_openheadAndemb_{checkpoint_id}'
        ...
        # todo
        for checkpoint_id in sorted(remove_id_list):
            checkpoint_path = checkpoint_dir / f'ft_Llama2-70B_GSM8K_lr5e-5_lisa_random4_openEmdAndHead_k100_3epoch_{checkpoint_id}'
            print('-', checkpoint_path)
