...
#       ${lora_args} \
#       --use_ram_optimized_load False \
#       --prompt_structure "###Question:怎么包饺子？ ###Answer: 1.准备好饺子皮和馅料，馅料可以根据个人喜好选择。2.饺子皮放入馅料后对折成半圆形，先将中间捏住，然后用左手从左边边缘开始向中间捏褶，右手从右边边缘开始向中间捏褶，饺子两半完全合上之后将边缘捏实弯曲呈元宝状。3.将包好的饺子放在沾了干面粉的盘子上，防止粘连。4.煮饺子时，将水烧开后放入适量的饺子，用勺子轻轻搅动防止沉底。水再次烧开后加入一些冷水，重复三次后就可以捞出饺子了。5.配上自己喜欢的醋、辣椒油等调料，就可以享用美味的饺子了。 ###Question: {input_text} ###Answer:" \
#       --max_new_tokens 400\
#       --end_string "###"
