            ...
            for p in filter(lambda p: exists(p.grad), group['params']):
                ...
                for layer, shape in self.layer_shape.items():
                    ...
                    if layer_start_global < end_param and layer_end_global > start_param:
                        ...
                        p.data[start_idx:end_idx].add_(update[start_idx:end_idx], alpha=-lr)

                        print_update_norm = torch.norm(update[start_idx:end_idx])
                        print(f"\nlocal rank {os.environ.get('LOCAL_RANK', '0')}, layer {layer}, shape {shape}, start {start_idx}, end {end_idx} weight norm {weight_norm}, update norm {print_update_norm}, grad norm {grad_norm}")
