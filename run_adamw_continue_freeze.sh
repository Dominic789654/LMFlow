...
run_name="ft_gpt2_adamw_C4_6e-3_continue_freeze_experiment"
lr=6e-3
bs=200
...
echo $random_layer
activate_layers="--activate_layers  0 $random_layer "
...
# 生成长度为10的随机列表，元素范围是1到11
random_list=($(shuf -i 1-10 -n 10))

# 打印生成的随机列表
echo "Generated list: ${random_list[*]}"


run_name="ft_gpt2_adamw_C4_3e-2_continue_freeze_experiment_activate0-11"
lr=3e-2
bs=200
per_device_eval_batch_size=1
use_lora=0
...
random_layer=${random_list[$((selected_portion-1))]}
echo $random_layer
activate_layers="--activate_layers  0  $random_layer 11"
echo $activate_layers
freeze_percentage=90
freeze_strategy="random"
local_seed=$RANDOM
